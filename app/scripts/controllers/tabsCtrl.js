...
var tabsCtrl = function($scope, globalService, $translate, $sce) {
    ...
    $scope.changeNode = function(key) {
        ...
        ajaxReq.getCurrentBlock(function(data) {
            if (data.error) {
                ...
                $scope.notifier.info( globalFuncs.successMsgs[5] + '.</strong> <br /> 当前网络: <strong> 萌娘工坊【' + $scope.nodeType + '】 </strong> 当前连入工坊全节点： <strong>' + $scope.nodeService + '.</strong>', 5000)
            }
        });
    ...
    $scope.addCustomNodeToList = function(nodeInfo) {
        var tempObj = null;
        if (nodeInfo.options == 'moeh') tempObj = JSON.parse(JSON.stringify(nodes.nodeList.moeh));
